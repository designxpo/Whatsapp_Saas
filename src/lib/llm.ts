...
export async function generateReply(history: { role: "user" | "assistant"; body: string }[], phone?: string, agentId?: string | null): Promise<ReplyResult> {
  ...
  try {
    chunks = await retrieve(lastUser.body, 6);
  } catch (err) {
