...
export interface Usage { contacts: number; messages: number; channels: number; seats: number }
...
export async function getTenantUsage(tenantId: string): Promise<Usage> {
  ...
  const [contacts, sendLog, convOut, channels, seats] = await Promise.all([
...
const LIMIT_KEY: Record<Resource, keyof PlanLimits> = {
  contacts: "contacts", messages: "messages_per_month", channels: "channels", seats: "team_seats",
};
const USAGE_KEY: Record<Resource, keyof Usage> = {
  contacts: "contacts", messages: "messages", channels: "channels", seats: "seats",
};
...
const LABEL: Record<Resource, string> = { contacts: "contacts", messages: "monthly messages", channels: "channels", seats: "team seats" };
