...
const SUPPORT_RE = /\b(my (?:account|login|payment|enrol?ment|batch|class|order|booking|appointment|subscription|delivery)|can'?t (?:login|log\s?in|access)|not (?:received|working|able to)|refund|reschedul\w*|missed (?:my|the) (?:class|session|appointment|delivery|slot)|recording|certificate not|didn'?t (?:get|receive)|issue with|problem with|complaint)\b/i;
const HIGH_INTENT_RE = /\b(enroll?|enrol(?:ment|ling)?|admission|admit|join(?:ing)?|register|registration|sign\s?up|pay(?:ment)?|emi|instal?ment|book(?:ing)?|reserve|reservation|order(?:ing)?|buy(?:ing)?|purchase|checkout|add to cart|start (?:the )?(?:course|order|booking)|how (?:do|to) (?:i )?(?:apply|join|enroll?|register|buy|order|book)|i want to (?:join|enroll?|register|start|buy|order|book)|ready to (?:join|start|pay|buy|order|book)|call\s?back|schedule a (?:call|visit|appointment)|free demo|book (?:a demo|an appointment|a table|a room|a visit)|get a quote)\b/i;
...
const EVALUATING_RE = /\b(compare|comparison|vs\.?|versus|difference between|which (?:course|program|product|plan|package|property|one|is better)|better|best (?:course|option|product|plan|deal|for)|worth it|syllabus|curriculum|placement|salary|package details|certificate|certification|eligib\w*|duration|how long|reviews?|ratings?|outcomes?|job (?:guarantee|assistance|support)|warranty|return policy|refund policy|cancellation policy|what'?s included|features|specifications?|after (?:the )?(?:course|purchase|booking))\b/i;
const BROWSING_RE = /\b(what (?:is|are|do)|tell me (?:about|more)|do you (?:have|offer|provide)|information|details about|explain|about (?:your|the|this) (?:course|program|institute|company|product|service|business)|who are you|courses?\b|programs?\b|products?\b|services?\b)\b/i;
...
const STAGE_GUIDANCE: Record<Stage, string> = {
  ...
  evaluating: "They're comparing options / weighing fit — give concrete specifics (price, duration/availability, what's included) from the business context, compare honestly, and highlight what suits them. Offer to connect them with our team for a tailored recommendation.",
  high_intent: "They're ready to act — be concrete and action-oriented: give the exact next step to buy/book/enrol, share the relevant link, and offer a callback from our team. Don't bury the lead under extra information.",
  objection: "They have a hesitation (price / timing / doubt) — acknowledge it, address it with value (payment options, outcomes, flexibility), and reassure without pressure.",
