...
// to receive agent replies. No `${` / backticks below so it survives this template.
export async function GET(req: Request, { params }: { params: Promise<{ siteKey: string }> }) {
  const { siteKey } = await params;
  const base = new URL(req.url).origin;
  const cfg = JSON.stringify({ siteKey, base });
...
"  var since = null, open = false, timer = null, seen = {};\n" +
"  var BRAND = '#0783fd';\n" +
"  var css = '' +\n" +
"   '.twc-btn{position:fixed;bottom:20px;right:20px;width:56px;height:56px;border-radius:50%;background:' + BRAND + ';color:#fff;border:none;cursor:pointer;box-shadow:0 6px 24px rgba(0,0,0,.18);z-index:2147483000;display:flex;align-items:center;justify-content:center;}' +\n" +
...
"  panel.innerHTML = '<div class=\"twc-head\"><span>Chat with us</span><button aria-label=\"Close\">&times;</button></div>' +\n" +
...
"  function toggle(o){ open = (o===undefined?!open:o); panel.className='twc-panel'+(open?' open':''); if(open){ input.focus(); poll(); if(!timer) timer=setInterval(poll, 4000); } }\n" +
