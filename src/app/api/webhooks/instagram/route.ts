...
import { getChannelByIgId, getChannelById, repairIgChannelId, effectiveAgentId, effectiveKbScope, type Channel } from "@/lib/channels";
...
async function aiRespond(channel: Channel, conv: Conversation, userText: string, commentId?: string) {
  ...
  const r = await generateReply(history.map(h => ({ role: h.role, body: h.body.replace(/^\[comment\] /, ""), mediaUrl: h.mediaUrl, mediaType: h.mediaType })), conv.phone, effectiveAgentId(conv, channel), tid, effectiveKbScope(conv, channel), askPhone, commentId ? undefined : { platform: "instagram", conversationId: conv.id }, "instagram");
...
async function aiThreadReply(channel: Channel, watch: CommentWatch, fu: { commentId: string; text: string; fromId: string; fromUsername: string }) {
  ...
  const r = await generateReply(history, conv.phone, effectiveAgentId(conv, channel), tid, effectiveKbScope(conv, channel), false, undefined, "instagram");
