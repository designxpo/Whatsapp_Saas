...
const DRAFT_TOOL: ChatTool = {
  name: "draft_campaign",
  description: "Draft the Meta ad campaign. Call this as soon as you know: the goal, what they're advertising, a budget (a total OR a per-day amount), and the duration (a number of days OR that it runs continuously). Fill the rest with sensible defaults if unstated.",
  params: [
    ...
    { name: "budgetTotal", description: "TOTAL budget as a plain number in major units, e.g. 5000. Provide this OR dailyBudget — not both. Omit if the user gave a per-day amount." },
    { name: "dailyBudget", description: "PER-DAY budget as a plain number in major units, e.g. 500. Use this when the user gives a daily spend instead of a total. Provide this OR budgetTotal." },
    { name: "ongoing", description: "true if the campaign has NO end date / runs continuously (e.g. the user says 'no end date', 'ongoing', 'until I stop it'). When true, `days` is not required." },
    { name: "days", description: "How many days to run, as a plain number. Omit when ongoing is true." },
  ...
  required: ["goal", "product"],
};

const SYSTEM = [
  "You are a friendly Meta (Facebook/Instagram) ads assistant helping a business owner set up an ad campaign by chatting.",
  "Learn, through short natural questions, what you need: the GOAL (WhatsApp chats / Messenger chats / website visits), WHAT they're advertising, their BUDGET (either a total, or a per-day amount), the DURATION (a number of days, or whether it runs continuously with no end date), which COUNTRIES, and whether they want to test MULTIPLE audiences (ad sets).",
  ...
  "IMPORTANT: budget and duration can each be given two ways. If the user gives a PER-DAY budget (e.g. '500 a day'), use dailyBudget — do NOT ask for a total. If the user says the campaign has NO END DATE / is ongoing (e.g. 'no end date', 'run it continuously'), set ongoing=true and do NOT ask how many days. Never re-ask for something the user already answered.",
  "As SOON as you know the goal, what they're advertising, a budget (total OR per-day), and a duration (a day count OR that it's ongoing), CALL the draft_campaign function — do not keep asking for nice-to-haves. For the WEBSITE goal you also need the landing-page URL.",
  "When the user attaches or pastes a prepared brief, treat it as the source of truth: pull the goal, budget (total or per-day), duration (or ongoing), countries, audience, creative format, and any URL straight out of it. Acknowledge in ONE short sentence what you understood, then call draft_campaign — pass the brief's full text in the `brief` field. Only ask for a required detail (e.g. the website URL for a WEBSITE goal) if it is genuinely absent from BOTH the brief and the conversation. Never re-ask for anything already provided earlier in the chat.",
...
export async function POST(req: Request) {
  ...
  try {
    ...
    if (call) {
      ...
      const budgetTotal = num(a.budgetTotal);
      const dailyBudget = num(a.dailyBudget);
      const days = Number(String(a.days ?? "").replace(/[^0-9]/g, "")) || 0;
      const ongoing = a.ongoing === true || String(a.ongoing).toLowerCase() === "true" || (!days && !budgetTotal && dailyBudget > 0);
      // Defensive: the model called too early without any budget — ask for it.
      if (!budgetTotal && !dailyBudget) return NextResponse.json({ reply: "How much would you like to spend — a total, or a daily budget (e.g. ₹500/day)?" });
      ...
      const dur = plan.ongoing ? "running continuously (no end date)" : `for ${plan.days} days`;
      const reply = `Done — I've drafted "${plan.campaignName}": ${plan.adSets.length} ad set${plan.adSets.length === 1 ? "" : "s"}, ${currency} ${plan.dailyBudget}/day ${dur}. Review it, add an image, and publish on the panel that just opened. 👉`;
