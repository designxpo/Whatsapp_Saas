...
async function report(ch: Channel): Promise<Report> {
  ...
  const idMatches = !live.id || live.id === ch.igUserId;
  if (!idMatches) {
    return { ...base, messages: subs.messages, comments: subs.comments, fields: subs.fields, idMatches: false, liveId: live.id, status: "wrong-id",
      detail: `This account is stored under id ${ch.igUserId}, but Instagram calls it ${live.id}. Nothing inbound can match — reconnect the account to fix it.` };
  }
  if (!subs.ok) {
    return { ...base, messages: false, comments: false, fields: [], idMatches: true, liveId: live.id, status: "error",
      detail: `Meta wouldn't say what this account is subscribed to (${subs.error}).`, error: subs.error };
  }
  if (!subs.messages) {
    return { ...base, messages: false, comments: subs.comments, fields: subs.fields, idMatches: true, liveId: live.id, status: "error",
  ...
  if (!subs.comments) {
    return { ...base, messages: true, comments: false, fields: subs.fields, idMatches: true, liveId: live.id, status: "dms-only",
  ...
  return { ...base, messages: true, comments: true, fields: subs.fields, idMatches: true, liveId: live.id, status: "ok",
