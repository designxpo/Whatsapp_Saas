...
export async function GET(req: Request) {
  ...
  const { data: convRow } = await db().from("wa_conversations")
    ...
    .eq("phone", digits).order("created_at", { ascending: false }).limit(1).maybeSingle();
  ...
  if (convRow) {
    const [{ data: msgs }, inb, outb] = await Promise.all([
      db().from("wa_conv_messages").select("role, body, source, created_at").eq("conversation_id", convRow.id).order("created_at", { ascending: false }).limit(10),
      db().from("wa_conv_messages").select("*", { count: "exact", head: true }).eq("conversation_id", convRow.id).eq("role", "user"),
      db().from("wa_conv_messages").select("*", { count: "exact", head: true }).eq("conversation_id", convRow.id).eq("role", "assistant"),
    ]);
  ...
  const { data: logRows } = await db().from("wa_send_log")
    .select("campaign_id, status, sent_at").eq("phone", digits).order("sent_at", { ascending: false }).limit(15);
  const campaignIds = [...new Set((logRows ?? []).map(r => r.campaign_id as string).filter(Boolean))];
  const names = new Map<string, string>();
  if (campaignIds.length) {
    const { data: camps } = await db().from("wa_campaigns").select("id, name, template_name").in("id", campaignIds);
  ...
  const { data: linkRows } = await db().from("wa_links")
    .select("target_url, clicks, first_clicked_at").eq("phone", digits).gt("clicks", 0).limit(10);
...
export async function PATCH(req: Request) {
  ...
  if (!digits) return NextResponse.json({ error: "phone required" }, { status: 400 });
  await updateContactProfile(digits, { name: body.name, email: body.email, tags: body.tags, attributes: body.attributes });
