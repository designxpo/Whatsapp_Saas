...
function NodeCard({ icon, title, body, accent = false, className = "" }: { icon: string; title: string; body: string; accent?: boolean; className?: string }) {
  return (
    <div className={`rounded-2xl border bg-white p-4 shadow-[0_10px_30px_-14px_rgba(24,119,242,0.35)] ${accent ? "border-[#1877F2]/40 ring-1 ring-[#1877F2]/15" : "border-slate-200"} ${className}`}>
      <div className="flex items-center gap-3">
        <span className={`flex h-9 w-9 items-center justify-center rounded-xl ${accent ? "bg-[#1877F2] text-white animate-pulsesoft" : "bg-[#1877F2]/10 text-[#1877F2]"}`}><Ico name={icon} /></span>
        <div>
      ...
      <Reveal className="relative mx-auto mt-12 max-w-3xl">
        ...
        <div className="relative flex flex-col items-center">
          ...
          <div aria-hidden className="relative hidden h-7 w-full max-w-2xl md:block">
            <span className="absolute top-0 h-px bg-[#1877F2]/40" style={{ left: "16.666%", right: "16.666%" }} />
            {[16.666, 50, 83.333].map(x => <span key={x} className="absolute top-0 h-7 w-px bg-[#1877F2]/40" style={{ left: `${x}%` }} />)}
          </div>
...
function Connector() {
  return (
    <svg width="2" height="36" viewBox="0 0 2 36" className="my-1 overflow-visible">
      <line x1="1" y1="0" x2="1" y2="36" stroke="#1877F2" strokeWidth="2" strokeOpacity="0.5" className="animate-dash" />
    </svg>
  );
}
      ...
      <div className="rounded-[28px] bg-slate-50 px-5 py-12 sm:px-10">
        ...
        <Reveal className="relative mt-14">
          <div className="relative grid grid-cols-2 gap-x-6 gap-y-14 md:grid-cols-4 md:gap-x-8">
            ...
            <span aria-hidden className="absolute top-7 hidden h-0.5 rounded bg-gradient-to-r from-[#1877F2]/20 via-[#1877F2]/50 to-[#1877F2]/20 md:block" style={{ left: "12.5%", right: "12.5%" }} />
            {steps.map((s, i) => (
              <Reveal key={s.n} delay={i * 140} className="relative flex flex-col items-center text-center">
                <span className="relative z-10 flex h-14 w-14 items-center justify-center rounded-2xl bg-[#1877F2] text-white shadow-[0_12px_26px_-8px_rgba(24,119,242,0.8)] ring-[6px] ring-slate-50">
                  <Ico name={s.icon} />
                </span>
                <span className="mt-4 inline-flex items-center gap-1 rounded-full bg-white px-3 py-1 text-[11px] font-bold text-[#1877F2] ring-1 ring-[#1877F2]/15">{s.meta}</span>
                <h3 className="mt-3 text-base font-extrabold text-slate-900"><span className="text-[#1877F2]">{s.n}.</span> {s.title}</h3>
