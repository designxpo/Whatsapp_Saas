      ...
      <Reveal className="relative mx-auto mt-12 max-w-3xl">
        ...
        <div className="relative flex flex-col items-center">
          ...
          <div aria-hidden className="relative hidden h-7 w-full max-w-2xl md:block">
            <span className="absolute top-0 h-px bg-[#0783fd]/40" style={{ left: "16.666%", right: "16.666%" }} />
            {[16.666, 50, 83.333].map(x => <span key={x} className="absolute top-0 h-7 w-px bg-[#0783fd]/40" style={{ left: `${x}%` }} />)}
          </div>
...
function Connector() {
  return (
    <svg width="2" height="36" viewBox="0 0 2 36" className="my-1 overflow-visible">
      <line x1="1" y1="0" x2="1" y2="36" stroke="#0783fd" strokeWidth="2" strokeOpacity="0.5" className="animate-dash" />
    </svg>
  );
}
          ...
          <div className="relative grid grid-cols-2 gap-x-6 gap-y-14 md:grid-cols-4 md:gap-x-8">
            ...
            {steps.map((s, i) => (
              <Reveal key={s.n} delay={i * 140} className="relative flex flex-col items-center text-center">
                ...
                <h3 className="mt-3 text-base font-extrabold text-slate-900"><span className="text-[#0783fd]">{s.n}.</span> {s.title}</h3>
