...
function Wordmark({ dark = false }: { dark?: boolean }) {
  return (
    <Link href="/" className="flex items-center gap-2">
      <span className="flex h-8 w-8 items-center justify-center rounded-lg bg-[#0164ff] text-white"><Bot className="h-4 w-4" /></span>
    ...
    <header className="sticky top-0 z-50 border-b border-slate-100 bg-white/90 backdrop-blur-xl">
      <nav className="mx-auto flex h-16 w-full max-w-6xl items-center justify-between px-5 sm:px-8">
        ...
        <div className="hidden items-center gap-8 md:flex">
          {NAV.map(l => (
            <Link key={l.href} href={l.href} className="text-sm font-semibold text-slate-600 transition-colors hover:text-[#0164ff]">{l.label}</Link>
          ))}
        </div>
        <div className="hidden items-center gap-3 md:flex">
          <Link href={SITE.domainCta.login} className="text-sm font-semibold text-slate-600 transition-colors hover:text-[#0164ff]">Sign in</Link>
          <Link href={SITE.domainCta.trial} className="rounded-full border-2 border-[#0164ff] px-4 py-1.5 text-sm font-bold uppercase tracking-wide text-[#0164ff] transition-colors hover:bg-[#0164ff] hover:text-white">Sign up now</Link>
        </div>
      ...
      {open && (
        <div className="border-t border-slate-100 bg-white px-5 py-4 md:hidden">
          <div className="flex flex-col gap-1">
            {NAV.map(l => (
              <Link key={l.href} href={l.href} onClick={() => setOpen(false)} className="rounded-lg px-2 py-2.5 text-sm font-semibold text-slate-600 hover:bg-slate-50 hover:text-[#0164ff]">{l.label}</Link>
            ))}
            <div className="mt-2 flex flex-col gap-2">
              ...
              <Link href={SITE.domainCta.trial} className="rounded-full bg-[#0164ff] px-4 py-2 text-center text-sm font-bold text-white">Sign up now</Link>
            </div>
          </div>
        </div>
      )}
...
export function SiteFooter() {
  ...
  return (
    <footer className="bg-[#0164ff] text-white">
      <div className="mx-auto grid w-full max-w-6xl gap-10 px-5 py-14 sm:px-8 md:grid-cols-[1.4fr_1fr_1fr_1.4fr]">
        ...
        <div>
          ...
          <form className="mt-3 flex flex-col gap-2" action="/signup">
            ...
            <button type="submit" className="rounded-full bg-white px-4 py-2.5 text-sm font-bold text-[#0164ff] transition-colors hover:bg-slate-100">Subscribe</button>
          </form>
        </div>
      </div>
      ...
      {FAQS.map((f, i) => (
        <div key={f.q}>
          <button onClick={() => setOpen(open === i ? null : i)} className="flex w-full items-center justify-between gap-4 px-6 py-5 text-left">
            ...
            <ChevronDown className={`h-4 w-4 shrink-0 text-[#0164ff] transition-transform ${open === i ? "rotate-180" : ""}`} />
          </button>
