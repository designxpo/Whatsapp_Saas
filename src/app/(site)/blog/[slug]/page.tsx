    ...
    <>
      <section className="relative overflow-hidden">
        ...
        <Container className="relative pt-16 pb-4">
          <Link href="/blog" className="inline-flex items-center gap-1.5 text-sm text-slate-500 transition-colors hover:text-[#0164ff]"><ArrowLeft className="h-4 w-4" /> All articles</Link>
          <div className="mx-auto mt-8 max-w-3xl text-center">
            <div className="flex items-center justify-center gap-2 text-xs text-slate-500">
              <span className="rounded-full bg-[#0164ff]/10 px-2.5 py-1 font-bold text-[#0164ff]">{post.category}</span>
            ...
            <div className="mt-5 grid gap-4 sm:grid-cols-2">
              {more.map(p => (
                <Link key={p.slug} href={`/blog/${p.slug}`} className="rounded-xl border border-slate-200 bg-white p-5 transition-shadow hover:shadow-[0_10px_24px_-12px_rgba(24,119,242,0.3)]">
                  <div className="text-xs font-bold text-[#0164ff]">{p.category}</div>
