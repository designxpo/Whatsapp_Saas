          ...
          {POSTS.map(p => (
            <Link key={p.slug} href={`/blog/${p.slug}`}
              ...
              <div className="flex items-center gap-2 text-xs text-slate-500">
                <span className="rounded-full bg-[#0164ff]/10 px-2.5 py-1 font-bold text-[#0164ff]">{p.category}</span>
                <span>{p.readTime}</span>
              </div>
              <h3 className="mt-4 text-lg font-bold leading-snug text-slate-900 group-hover:text-[#0164ff]">{p.title}</h3>
