          ...
          {POSTS.map(p => (
            <Link key={p.slug} href={`/blog/${p.slug}`}
              ...
              <div className="flex items-center gap-2 text-xs text-slate-500">
                <span className="rounded-full bg-[#1877F2]/10 px-2.5 py-1 font-bold text-[#1877F2]">{p.category}</span>
                <span>{p.readTime}</span>
              </div>
              <h3 className="mt-4 text-lg font-bold leading-snug text-slate-900 group-hover:text-[#1877F2]">{p.title}</h3>
